...
時間指定（「何分前」）で絞り込まず、毎回全コメントを取得して判定する。
...
gh api --paginate repos/{owner}/{repo}/pulls/<PR番号>/comments?per_page=100

# PRのissueコメント（会話欄コメント）を全件取得
gh api --paginate repos/{owner}/{repo}/issues/<PR番号>/comments?per_page=100
```

### 判定ルール（ステータスマーカー運用）

- コメント本文に以下のマーカーがある場合は「対応済み」として扱う
   - `<!-- resolved: true -->`（修正完了）
   - `<!-- ignored: true -->`（対応不要として明示）
   - `<!-- wontfix: true -->`（対応しない判断を明示）
- マーカーが無いコメントのみを「未対応」として扱う
- `gh pr view --comments` は補助的に使ってよいが、最終判定は `gh api --paginate` で取得した全件で行う

### 運用ルール

- 対応完了コメントには、返信の末尾に `<!-- resolved: true -->` を付与する
- 対応不要・対応しない判断をした場合は、理由を書いたうえでそれぞれのマーカーを付与する
- 修正を行った場合は、必要に応じて返信内に修正コミットへのリンク（または短縮SHA）を1行添えてよい
- これにより次回確認時は、マーカー付きコメントを自動的にスキップして未対応のみ確認する
